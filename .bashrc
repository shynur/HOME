...
PS1=\\e[0m\\n'Exited with ''`last_exit_code=$?; case $last_exit_code in 0) echo \e[32m0\e[0m;; *) echo -n "\e[31m$last_exit_code\e[0m";;& 2) echo " (incorrect usage, generally invalid options or missing arguments)";; 126) echo " (command not executable)";; 127) echo " (command not found)";; 129|1[3-9]?|2??) sig_spec=$(gcc -dM -E - <<<"#include <signal.h>" | command grep -E "^[[:blank:]]*#[[:blank:]]*define[[:blank:]]+SIG[A-Z0-9]+[[:blank:]]+[0-9]+" | sed "s/^[[:blank:]]*#[[:blank:]]*define//" | awk '\''128+$2=='\''$last_exit_code'\''{print$1}'\''); [ $sig_spec ]&&echo " ($sig_spec)";;esac`'';  ''\e[1;35m`((\j))&&echo \j`\e[0m`((\j))&&echo job\`((\j>1))&&echo s\` remain`'\\n'load:`\`[ -x /bin/emacs ]&&echo /bin/emacs||echo emacs\` -x <(echo "(princ (format \"\e[1;33m%s%%\e[0m\" (nth 0 (load-average))) t)")`'\ '`command free -h | command grep '\''^Mem:'\'' | awk '\''{print $3"/"$2}'\'' | awk '\''{print "mem:\e[1;33m"$1"\e[0m"}'\''`'\ '`command free -h | command grep '\''^Swap:'\'' | awk '\''{print $3"/"$2}'\'' |command grep -vF /0 | awk '\''{print "swap:\e[1;33m"$1"\e[0m "}'\''`''real:\e[1;33m$[SECONDS/60]m$[SECONDS%60]s\e[0m `cat /tmp/.shynur.bash.times.$$.txt | paste --serial | awk -F'\''[ms[:space:]]+'\'' '\''{print ($1*60+$2)+($5*60+$6)" "($3*60+$4)+($7*60+$8)}'\'' | python3 -c "u,s=[float(s)for s in input().split()];print(int(u)//60,int(u)%60,int(s)//60,int(s)%60)"| awk '\''{print "user:\e[1;33m"$1"m"$2"s\e[0m sys:\e[1;33m"$3"m"$4"s\e[0m"}'\'\`'`shopt -s extglob; for d in ~ /var/www/html /opt /bin; do [[ \w/ = ${d%%+(/)}/* ]] && break; done &&echo -en " "du:\\\\\\e[1\\;33m &&command du --human-readable --summarize 2>/dev/null | awk "{print \\\\$1}" | head -c-1 &&echo -en \\\\\\e[0m`'\\n'\e[1;32m'\\u'\e[0m'@'\e[1;32m'\\H'\e[0m':'\e[1;34m'\\w'\e[0m'\ \\#'`(((11<=\#&&\#<=13))&&echo th)||(((\#%10==1))&&echo st)||(((\#%10==2))&&echo nd)||(((\#%10==3))&&echo rd)||echo th`'\ [\\d\ '\D{%I:%M:%S %p}']'$((`git rev-parse --is-inside-work-tree 2>/dev/null`||`git rev-parse --is-inside-git-dir 2>/dev/null`)&&echo -e "" \e[38\;2\;241\;78\;50mGit\e[0m:`[ -z $(git branch --show-current) ]&&git describe --all||git branch --show-current`)'\\n'`((SHLVL-1))&&echo \\\\[$SHLVL] ""`''\$ '\\[\\e[3m\\]
# PS1 还会像 Emacs minibuffer-depth-indicate-mode 那样显示递归层级.
PS2='\[\a\]'
PS0='\e[0m'
...
# 打开小键盘的 NumLock 指示灯.
if [ `command uname` = Linux ] && [[ `tty` =~ /dev/tty[0-9]+ ]]; then
