...
PS1=\\e[0m\\nreturned\ '\e[1;31m'\$?'\e[0m'';  ''\e[1;35m`((\j))&&echo \j`\e[0m`((\j))&&echo job\`((\j>1))&&echo s\` remain`'\\n'load:`\`[ -x /bin/emacs ]&&echo /bin/emacs||echo emacs\` -x <(echo "(princ (format \"\e[1;33m%s%%\e[0m\" (nth 0 (load-average))) t)")`'\ '`command free -h | egrep '\''^(Mem|Swap):'\'' | head -n 3|tail -n 2 | awk '\''{print $3"/"$2}'\'' | paste --serial | awk '\''{print "mem:\e[1;33m"$1"\e[0m swap:\e[1;33m"$2"\e[0m"}'\''`'\ 'real:\e[1;33m$[SECONDS/60]m$[SECONDS%60]s\e[0m `cat /tmp/.shynur.bash.times.$$.txt | paste --serial | awk -F'\''[ms[:space:]]+'\'' '\''{print ($1*60+$2)+($5*60+$6)" "($3*60+$4)+($7*60+$8)}'\'' | python3 -c "u,s=[float(s)for s in input().split()];print(int(u)//60,int(u)%60,int(s)//60,int(s)%60)" | awk '\''{print "user:\e[1;33m"$1"m"$2"s\e[0m sys:\e[1;33m"$3"m"$4"s\e[0m"}'\'\`'`for d in ~ /var/www/html/ /opt/ /bin/; do [[ \w/ = $d* ]] && break; done &&echo -en " "du:\\\\\\e[1\\;33m &&command du --human-readable --summarize 2>/dev/null | awk "{print \\\\$1}" | head -c-1 &&echo -en \\\\\\e[0m`'\\n'\e[1;32m'\\u'\e[0m'@'\e[1;32m'\\H'\e[0m':'\e[1;34m'\\w'\e[0m'\ \\#'`(((11<=\#&&\#<=13))&&echo th)||(((\#%10==1))&&echo st)||(((\#%10==2))&&echo nd)||(((\#%10==3))&&echo rd)||echo th`'\ [\\d\ '\D{%I:%M:%S %p}']'$((`git rev-parse --is-inside-work-tree 2>/dev/null`||`git rev-parse --is-inside-git-dir 2>/dev/null`)&&echo -e "" \e[38\;2\;241\;78\;50mGit\e[0m:`[ -z $(git branch --show-current) ]&&git describe --all||git branch --show-current`)'\\n'`((SHLVL-1))&&echo \\\\[$SHLVL] ""`''\$ '\\[\\e[3m\\]
